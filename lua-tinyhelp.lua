...
function _read()
    ...
    if _VERSION == "Lua 5.3" then
        fname = path.join(fpath,"lua_53.xml")
    elseif _VERSION == "Lua 5.2" then
        fname = path.join(fpath,"lua_52.xml")
    elseif _VERSION == "Lua 5.1" then
        fname = path.join(fpath,"lua_51.xml")
    end
...
function luahelp.help(query)
    ...
    for item in root[2]:childtags() do
        ...
        if title and (title:match(query)) then
            print(title)
            print(_clean(item[1]))
            found = true
        end
